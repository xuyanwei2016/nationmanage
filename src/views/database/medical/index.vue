  ...
  <el-container>
    ...
    <el-main>
      <div class="nav-buttons-container">
        <router-link
          :to="{ path:`create`,query:{articleType:38,articleName:'医论'}}"
        ...
        >
          ...
          <i
            v-if="authBtns.filter(i => i.idName=='batchExamineBtn').length>0 && authBtns.filter(i => i.idName=='batchExamineBtn')[0].icon"
            :class="[authBtns[authBtns.map(i => i.idName).indexOf('batchExamineBtn')].icon, 'el-icon--right']"
          ></i>
        </el-button>
        <el-button
          v-if="authBtns.map(i => i.idName).indexOf('editPriceBtn') >= 0"
          @click="editPriceDialog"
        >
          {{authBtns.filter(i => i.idName=='editPriceBtn')[0].name}}
          <i
            v-if="authBtns.filter(i => i.idName=='editPriceBtn').length>0 && authBtns.filter(i => i.idName=='editPriceBtn')[0].icon"
            :class="[authBtns[authBtns.map(i => i.idName).indexOf('editPriceBtn')].icon, 'el-icon--right']"
          ></i>
        </el-button>
        <el-button v-if="authBtns.map(i => i.idName).indexOf('editTypeBtn') >= 0" @click="editType">
          {{authBtns.filter(i => i.idName=='editTypeBtn')[0].name}}
          <i
            v-if="authBtns.filter(i => i.idName=='editTypeBtn').length>0 && authBtns.filter(i => i.idName=='editTypeBtn')[0].icon"
            :class="[authBtns[authBtns.map(i => i.idName).indexOf('editTypeBtn')].icon, 'el-icon--right']"
          ></i>
        </el-button>
        <el-button
          v-if="authBtns.map(i => i.idName).indexOf('setBtn') >= 0"
          @click="editReadDialog"
        >
        ...
        <el-button
          v-if="authBtns.map(i => i.idName).indexOf('showonBtn') >= 0"
          @click="editStatus(1)"
        >
          {{authBtns.filter(i => i.idName=='showonBtn')[0].name}}
          <i
            v-if="authBtns.filter(i => i.idName=='showonBtn').length>0 && authBtns.filter(i => i.idName=='showonBtn')[0].icon"
            :class="[authBtns[authBtns.map(i => i.idName).indexOf('showonBtn')].icon, 'el-icon--right']"
          ></i>
        </el-button>
        <el-button
          v-if="authBtns.map(i => i.idName).indexOf('setoutBtn') >= 0"
          @click="editStatus(2)"
        >
          {{authBtns.filter(i => i.idName=='setoutBtn')[0].name}}
          <i
            v-if="authBtns.filter(i => i.idName=='setoutBtn').length>0 && authBtns.filter(i => i.idName=='setoutBtn')[0].icon"
            :class="[authBtns[authBtns.map(i => i.idName).indexOf('setoutBtn')].icon, 'el-icon--right']"
          ></i>
        </el-button>
        <el-button
      ...
      >
        ...
        >
          <template slot-scope="scope">
            <router-link
              :to="{ path:`create`, query:{id:scope.row.id,articleType:38,articleName:'医论'}}"
            ...
            <router-link
              :to="{ path:`create`, query:{detailsId:scope.row.id,articleType:38,articleName:'医论'}}"
            ...
            <router-link
              :to="{ path:`doctor/create_doctor`, query:{id:scope.row.id}}"
              v-if="authBtns.filter(i => i.idName=='relevant').length > 0"
            >
              <i
                :class="authBtns.filter(i => i.idName=='relevant')[0].icon"
                :title="authBtns.filter(i => i.idName=='relevant')[0].name"
              ></i>
            </router-link>
            <i
              :class="authBtns.filter(i => i.idName=='deleteBtn')[0].icon"
              :title="authBtns.filter(i => i.idName=='deleteBtn')[0].name"
              v-if="authBtns.filter(i => i.idName=='deleteBtn').length > 0 && scope.row.status != 1"
    ...
    <el-dialog title="批量修改价格" :visible.sync="dialogPrice" width="30%">
      <template>
        ...
        <el-form :model="dataForm" v-if="priceType == 1" :rules="rules" class="aa" ref="dataForm">
...
import "@riophae/vue-treeselect/dist/vue-treeselect.css";//6
export default {
  data() {
    return {
      //3
  ...
  methods: {
    ...
    editPrice() {//确认修改价格
      ...
      const data = {
        ids:ids,
        priceOption:this.priceType,
      }
      if(this.priceType == 1){//付费
    ...
    resetFilter(){//重置
      ...
      this.listQuery = {
        ...
        resourceType: 38,//文章类型 1理论 2药物 3方剂 4临床 5医案 6文化 7机构 8其他 9未分类 10蒙文

<template>
  <el-container>
    ...
    <div class="nav-buttons-container">
      <router-link :to="{ path:`addVideo`}" v-if="authBtns.filter(i => i.idName=='addBtn').length > 0">
    ...
    <template>
      <el-tabs v-model="activeName" type="card" @tab-click="listSatus">
        <!-- <el-tab-pane label="所有视频" name="books"></el-tab-pane> -->
    ...
    <el-table :key='tableKey' :data="list" v-loading="listLoading" @sort-change="handleSort" element-loading-text="给我一点时间" border highlight-current-row stripe @selection-change="chioceList" >
      ...
      <el-table-column label="资源名称" prop="name" min-width="250" align="center" :show-overflow-tooltip="true"></el-table-column>
      <el-table-column label="售价" min-width="140" align="center" >
        <template slot-scope="scope">
          <span v-if="scope.row.realPrice != 0">{{scope.row.realPrice}}</span>
          <span v-else>免费</span>
        </template>
      </el-table-column>
      <el-table-column label="创建时间" min-width="180" align="center" prop="uploadTime" sortable="custom"></el-table-column>
      <el-table-column label="时长" prop="timeLength" min-width="120" align="center" ></el-table-column>
      <el-table-column label="文件大小" prop="size" min-width="120" align="center"></el-table-column>
      ...
      <el-table-column class-name="small-padding fixed-width" label="操作" width="160" align="center">
        <template slot-scope="scope">
          <router-link :to="{ path:`addVideo`, query:{id:scope.row.id}}" v-if="authBtns.filter(i => i.idName=='editBtn').length > 0"><i :class="authBtns.filter(i => i.idName=='editBtn')[0].icon" :title="authBtns.filter(i => i.idName=='editBtn')[0].name"></i></router-link>
          <router-link :to="{ path:`addVideo`, query:{detailsId:scope.row.id}}" v-if="authBtns.filter(i => i.idName=='detailsBtn').length > 0"><i :class="authBtns.filter(i => i.idName=='detailsBtn')[0].icon" :title="authBtns.filter(i => i.idName=='detailsBtn')[0].name"></i></router-link>
          <i :class="authBtns.filter(i => i.idName=='deleteBtn')[0].icon" :title="authBtns.filter(i => i.idName=='deleteBtn')[0].name" v-if="authBtns.filter(i => i.idName=='deleteBtn').length > 0 && scope.row.status != 4&&scope.row.status != 2" @click="deleteBtn(scope.row.id)"></i>
        </template>
      </el-table-column>
    </el-table>
...
<script>
import { getListAPI, batchExamineAPI, batchDeleteAPI } from '@/api/medicalTreatment/videoManage'
...
export default {
  ...
  methods: {
    ...
    batchExamine(){//打开批量审核
      if(this.batchList.length > 0){
        ...
        if (status) {
          ...
          this.$message.error('只有未审核状态的数据才能进行审核哦');
        }
      } else{
    ...
    deleteBtn(id){//单条删除
      const ids = [];
      ids.push(id);
      confirm.apply(this,['确定要删除此条视频吗？']).then(() => {
    ...
    batchDelete(){//批量删除--不能删除已上架的资源
      if(this.batchList.length == 0){
        ...
        if(dataType){
          const ids = getMultipleSelectionIDs(this.batchList);
          confirm.apply(this,['确定要删除已选视频吗？']).then(() => {
            batchDeleteAPI(ids).then(res => {
              requestMsg.apply(this,[res.data,'删除'])
              this.getList()
            })

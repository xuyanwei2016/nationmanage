  ...
  <div class="app-container calendar-list-container">
    <div class="filter-container">
      <el-form inline class="form-inline">
        <el-form-item label="课程名称：" label-width="100px">
          <el-input @keyup.enter.native="handleFilter" v-model="listQuery.courseName" clearable placeholder="请输入课程名称"></el-input>
        </el-form-item>
    ...
    <div class="filter-container">
      <div class="nav-buttons-container">
        <router-link :to="{ path:`course/add_course` }"><el-button icon="el-icon-download" @click="addData">添加</el-button></router-link>
    ...
    <template>
      <el-tabs v-model="status" type="card" @tab-click="handleClick" tab-position="bottom">
        <el-tab-pane label="所有视频" name="total">所有课程视频</el-tab-pane>
    ...
    <el-table :key='tableKey' :data="list" v-loading="listLoading" element-loading-text="给我一点时间" border highlight-current-row stripe @selection-change="chioceList">
      ...
      <el-table-column label="更新期数" align="center">
        <template slot-scope="scope">
          <span>{{scope.row.updatePeriods}}</span>
        </template>
      </el-table-column>
      ...
      <el-table-column class-name="small-padding fixed-width" label="操作" width="200" align="center">
        <template slot-scope="scope">
          <router-link :to="{ path:`course/add_course`, query:{id:scope.row.id,isEdit:false} }"><i class="el-icon-zoom-in" title="查看"></i></router-link>
          <router-link :to="{ path:`course/add_course`, query:{id:scope.row.id} }"><i class="el-icon-edit" title="编辑"></i></router-link>
          <router-link :to="{ path:`course/set_course`, query:{id:scope.row.id} }"><i class="el-icon-setting" title="管理"></i></router-link>
        </template>
      </el-table-column>
    </el-table>
...
import { getListAPI, upDownAPI, batchDeleteAPI } from '@/api/resource/course'

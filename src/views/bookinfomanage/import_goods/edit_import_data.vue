<template>
  <div class="app-container">
    <h3>修改导入商品信息</h3>
    <el-form ref="dataForm" :model="temp" :rules="rules" label-position="right" label-width="120px" class="double-column-form clearfix">
      ...
      <el-form-item label="关联图片：" prop="relationPicture">
    ...
    <section class="related-goods" v-if="$route.query.booktype==1">
      <h3>套餐关联商品</h3>
...
import { getPublisherAPI, getNavAPI, getOptionAPI, getExpressAPI, getRelatedBooksAPI, getBooksAPI } from '@/api/book-infomation/package'
import { getInfoAPI, updateAPI } from '@/api/book-infomation/importGoods'
...
export default {
    ...
    return {
      ...
      rules: {
        ...
        pbook: {
          type: 'object',
          subTitle: [
            { type : 'string', trigger: 'change', max: 500, message: "最多500个字符",required: true },
            { whitespace: true, message: "未填写", trigger: 'change' }
          ],
          salePoint: [
            { type: 'string', trigger: 'change', message: "未填写", whitespace: true},
            { max: 100, message: "最多100个字符", trigger: 'change'}
          ],
  ...
  methods: {
    ...
    // 改变Tree选中节点时修改select
    setSelect(data, obj){
      this.temp.diytype = obj.checkedNodes.map(item => {
        return {diytypeid: item.id, code: item.code}
      })
    },
    // 删除多选标签设置Tree的选中节点
    setTree(tag){
      if (this.temp.diytype.length>0) {
        let checkedKeys = getMultipleSelectionIDs(this.temp.diytype,'diytypeid')
    ...
    dataSubmit(){
      if (this.$route.query.booktype == 1 && this.list.length===0) {
      ...
      this.$refs['dataForm'].validate((valid) => {
        if (valid) {
          ...
          tempData.cover = this.temp.picList.map(pic => {
            let {name, response} = pic
            return {name, url: response, flag: 0}
          },[])
          ...
          this.$route.query.booktype == 1 ? tempData.bookId = getMultipleSelectionIDs(this.list) : tempData
          // console.log(tempData);
          this.invokeAPI(updateAPI, tempData)
        } else {
          this.$message.warning("有未通过的验证")
        }
      })
    },
    invokeAPI(api, data){ //修改调接口
      api(data).then(res => {
        if(res.data.code === 0){
          this.$message.success(`修改导入商品信息成功`)
          this.$router.go(-1);
        } else{
          this.$message.error(`修改导入商品失败`)
        }
      })
    },
    getGoods(){
    ...
    getInfo(){
      getInfoAPI(this.$route.query.id).then(res => {
        if (res.data.code === 0) {
          ...
          this.$route.query.booktype == 1 ? this.getList() : true

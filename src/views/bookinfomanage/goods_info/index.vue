  ...
  <el-container>
    ...
    <el-main>
      <div class="nav-buttons-container">
        <router-link to="goods_info/add_edit_goods?type=create" v-if="authBtns.filter(i => i.idName=='addbtn').length > 0">
        ...
        <el-button v-if="authBtns.map(i => i.idName).indexOf('setbtn') >= 0" @click="massPresale">{{authBtns.filter(i => i.idName=='setbtn')[0].name}}<i v-if="authBtns.filter(i => i.idName=='setbtn').length>0 && authBtns.filter(i => i.idName=='setbtn')[0].icon" :class="[authBtns[authBtns.map(i => i.idName).indexOf('setbtn')].icon, 'el-icon--right']"></i></el-button>
        ...
        <el-button v-if="authBtns.map(i => i.idName).indexOf('hintbtn') >= 0" @click="handleHintWord">{{authBtns.filter(i => i.idName=='hintbtn')[0].name}}<i v-if="authBtns.filter(i => i.idName=='hintbtn').length>0 && authBtns.filter(i => i.idName=='hintbtn')[0].icon" :class="[authBtns[authBtns.map(i => i.idName).indexOf('hintbtn')].icon, 'el-icon--right']"></i></el-button>
      ...
      <el-form inline class="filter-form-inline">
        ...
        <el-form-item label="出版社">
          <el-select v-model="listQuery.publisherid">
      ...
      <el-table :key="tableKey" :data="list" @selection-change="handleSelectionChange" border highlight-current-row v-loading="listLoading" stripe element-loading-text="给我一点时间">
        ...
        <el-table-column label="书代号" min-width="130" align="center">
          <template slot-scope="scope">
        ...
        <el-table-column label="上下架时间" width="155" align="center">
          <template slot-scope="scope">
            <span>{{scope.row.bookstatus===0 ? scope.row.offShelvesTime : scope.row.shelvesTime}}</span>
          </template>
        </el-table-column>
        <el-table-column class-name="small-padding fixed-width" label="操作" width="140" align="center">
          <template slot-scope="scope">
            <router-link :to="`goods_info/add_edit_goods?type=update&id=${scope.row.id}`"  v-if="authBtns.filter(i => i.idName=='editbtn').length > 0">
      ...
      <el-dialog :title="textMap[dialogStatus]" :visible.sync="dialogFormVisible" width="40%" v-if="dialogFormVisible">
        ...
        <el-form :model="picTemp" ref="massForm" v-else-if="dialogStatus==='relate'" label-width="100px" style='margin-right:20px;'>
          <!-- <el-form-item label="添加图片：" prop="relationPic">
            <el-upload :action="`${domain}${requestPath.file}${uploadPath}`" list-type="picture-card" :on-remove="handleRemove" :on-preview="handlePicPreview" :on-success="handlePicSuccess" :before-upload="beforePicUpload" :disabled="uploading">
        ...
        <el-form :model="hintTemp" ref="massForm" v-else-if="dialogStatus==='hint'" label-width="110px" style='margin-right:20px;'>
          <el-form-item label="正版提示语：" prop="securitycodeGenuine" :rules="[{ required: true, whitespace: true, message: '未填写', trigger: 'change' }, { max: 200, message: '最多200字', trigger: 'change' }]">
            <el-input v-model="hintTemp.securitycodeGenuine" :autosize="{ minRows: 4 }" type="textarea" placeholder="最多200字"></el-input>
          </el-form-item>
          <el-form-item label="盗版提示语：" prop="securitycodePiracy" :rules="[{ required: true, whitespace: true, message: '未填写', trigger: 'change' }, { max: 200, message: '最多200字', trigger: 'change' }]">
            <el-input v-model="hintTemp.securitycodePiracy" :autosize="{ minRows: 4 }" type="textarea" placeholder="最多200字"></el-input>
          </el-form-item>
        </el-form>
        ...
        <span slot="footer" class="dialog-footer">
          ...
          <el-button v-else-if="dialogStatus==='hint'" @click="hintSubmit">确定</el-button>
...
import { getNavAPI, getListAPI, getPublisherAPI, massShelvesAPI, massSaleAPI, presaleAPI, deleteAPI, subtitleAPI, uploadFileAPI, relatePicAPI, batchHintAPI } from '@/api/book-infomation/goodsInfo'
...
export default {
  ...
  mounted() {
    if( ifUndefined(store.getters.goodsInfoSerKeys.pageNum) ){
      this.listQuery = store.getters.goodsInfoSerKeys
  ...
  methods: {
    hintSubmit() {
      this.$refs['massForm'].validate((valid) => {
        if (valid) {
          batchHintAPI(this.hintTemp).then(res => {
            requestMsg.apply(this, [res.data, `批量修改${this.textMap[this.dialogStatus]}` ])
    ...
    relateSubmit(){
      relatePicAPI(this.picTemp).then(res => {
        operationMsg.apply(this,[res.data.code,'关联图片'])
        res.data.status ? this.dialogFormVisible = false : true
        this.getList()
      })
    },
      ...
      this.$refs['massForm'].validate((valid) => {
        if (valid) {
          if (this.multipleSelection.some(item => item.fileStatus !== 0)) {
            confirm.apply(this,['所选商品已经有附件，如果继续将会覆盖，是否继续?']).then(() => {
              this.uploadSubmit()
            })
          } else {
            this.uploadSubmit()
    ...
    exportExcel(){
      ...
      let exportUrl = `${process.env.BASE_API}${requestPath.book}/book/export`
      paramStr ? exportUrl += `?${paramStr}` : true
      console.log(exportUrl)
      window.open(exportUrl)
      // paramStr ? window.open(`${process.env.BASE_API}${requestPath.book}/book/export?${paramStr}`) : window.open(`${process.env.BASE_API}${requestPath.book}/book/export`)
    },
    massDelete(){
      if(this.multipleSelection.length!=0){
        this.massSetting('确定删除选择的条目吗？', deleteAPI, this.ids, `批量删除${this.multipleSelection.length}项`)
      } else {
        this.$message.warning('请先选择需要设置的条目')
      }
    },
    ...
    singleSetting(row){
      ...
      if (row.bookstatus === 1) {
        data.bookstatus = 0
        this.massSetting('确定下架这本书吗？', massShelvesAPI, data, `下架`)
      } else {
        data.bookstatus = 1
        this.massSetting('确定上架这本书吗？', massShelvesAPI, data, `上架`)
      }
    },
    subtitleSubmit(){
      this.$refs['massForm'].validate((valid) => {
    ...
    handleSetting(statusKey, value){ //批量设置
      ...
      if(length!=0){
        ...
        if (statusKey === 'bookstatus') {
          value === 1 ? this.massSetting('确定上架这本/这些书吗？', massShelvesAPI, data, `批量上架${length}项`) : this.massSetting('确定下架这本/这些书吗？', massShelvesAPI, data, `批量下架${length}项`)
        } else if (statusKey === 'saleStatus') {
          value === 1 ? this.massSetting('确定设置这本/这些书可售吗？', massSaleAPI, data, `批量可售${length}项`) : this.massSetting('确定设置这本/这些书不可售吗？', massSaleAPI, data, `批量不可售${length}项`)
        } else {
          this.massSetting('确定取消这本/这些书的预售吗？', presaleAPI, data, `批量取消${length}项预售`)
        }
      } else {
        this.$message.warning('请先选择需要设置的条目')
      }
    ...
    handleFilter(){
      ...
      store.dispatch('SetGoodsInfoSerKeys',this.listQuery).then(res => {

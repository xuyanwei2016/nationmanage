<template>
  <div class="app-container calendar-list-container">
    ...
    <el-table :key='tableKey' :data="list" v-loading="listLoading" element-loading-text="给我一点时间" border highlight-current-row stripe @selection-change="chioceList" >
      ...
      <el-table-column label="资源来源" min-width="100" align="center" prop="accessMedium">
        <!--<template slot-scope="scope">
          &lt;!&ndash;<span>{{scope.row.releaseTime}}</span>&ndash;&gt;
        </template>-->
      </el-table-column>
      <el-table-column label="评论时间" min-width="130" align="center">
      ...
      <el-table-column class-name="small-padding fixed-width" width="80" label="操作"  align="center">
        <template slot-scope="scope">
          <router-link :to="{ path:`shareDetail`, query:{id:scope.row.id}}" v-if="authBtns.filter(i => i.idName=='review').length > 0"><i :class="authBtns.filter(i => i.idName=='review')[0].icon" :title="authBtns.filter(i => i.idName=='review')[0].name"></i></router-link>
...
<script>
import { getListAPI, batchExamineAPI, batchDeleteAPI, deleteAPI } from '@/api/society-manage/share'
